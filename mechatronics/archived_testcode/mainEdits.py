...
def main():
    ...
    serport1 = "0BEAA7CC5154465336202020FF0F1931"
    ...
    pollen_count = 0
    prev_cs = 0
    fill_amount = 1  # specified number of pollen before moving
    sg = 0
    lr = 0
    plant3 = "None"
    ...
    while True:
        ...
        # Position update
        delta1 = encoder1.read() - prev_value1 #replaced encoder call
        delta2 = encoder2.read() - prev_value2 #replaced encoder call
        ...
        if not sg:
            ...
            sg = (fsm.timer <= 120)  # Time is less than two minutes
        ...
        if fsm.tp:
            ...
            direction = 1
            QTPY2.set_status("go")
            [left,right,direction,success] = to_goal(position, goal) # TODO: change to_goal
            control_motors(left_motor, right_motor,direction,left,right) # TODO: fix this
            lr = success
        elif fsm.hp:
            goal = home
            timer = timer + (current_time - prev_time)
